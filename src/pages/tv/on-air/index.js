...
const OnTheAir = ()=>{
    ...
    const url = `${env.apiUrl}/tv/on_the_air?api_key=${env.tmdbApiKey}`;
    const NextPage = ()=>{
    ...
    useEffect(()=>{
        setTimeout(() => setLoading(true), 2000);
        fetch(`${env.apiUrl}/tv/on_the_air?api_key=${env.tmdbApiKey}&page=1`)
        .then(response => response.json())
        .then(nowPlayingData => {
            setNowPlayingData(nowPlayingData)
            console.log(nowPlayingData)
    ...
    const getData = async(pageCount)=>{
        setTimeout(() => setLoading(true), 2000);
        const nowPlayingDataResponse = await fetch(`${env.apiUrl}/tv/on_the_air?api_key=${env.tmdbApiKey}&page=${pageCount}`)
    ...
    return(
        <>
            <Head>
                <title>On The Air Tv Shows::Media</title>
...
//     const responsstvNowPlaying = await fetch(`${env.apiUrl}/tv/on_the_air?api_key=${env.tmdbApiKey}`);
//     const NowPlayingData = await responsstvNowPlaying.json();
//     return{
//       props:{
