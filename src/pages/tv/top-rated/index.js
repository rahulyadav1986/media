...
const TopRated = ()=>{
    ...
    const url = `${env.apiUrl}/tv/top_rated?api_key=${env.tmdbApiKey}`;
    const NextPage = ()=>{
    ...
    useEffect(()=>{
        setTimeout(() => setLoading(true), 2000);
        fetch(`${env.apiUrl}/tv/top_rated?api_key=${env.tmdbApiKey}&page=1`)
        .then(response => response.json())
        .then(topRatedData => {
            settopRatedData(topRatedData)
            console.log(topRatedData)
    ...
    const getData = async(pageCount)=>{
        setTimeout(() => setLoading(true), 2000);
        const topRatedDataResponse = await fetch(`${env.apiUrl}/tv/top_rated?api_key=${env.tmdbApiKey}&page=${pageCount}`)
    ...
    const HeroBackDrop = {
        backgroundImage : `url(${env.image_base_url}/original/${topRatedData?.results?.[Math.floor(Math.random() * 20)]?.backdrop_path})`
    }
    return(
        <>
            <Head>
                <title>Top Rated Tv Shows::Media</title>
...
//     const responsstvTopRated = await fetch(`${env.apiUrl}/tv/top_rated?api_key=${env.tmdbApiKey}`);
//     const topRatedData = await responsstvTopRated.json();
//     return{
//       props:{
