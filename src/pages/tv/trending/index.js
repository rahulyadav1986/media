...
const Trending = ()=>{
    ...
    useEffect(()=>{
        setTimeout(() => setLoading(true), 2000);
        fetch(`${env.apiUrl}/trending/tv/day?api_key=${env.tmdbApiKey}&page=1`)
        .then(response => response.json())
        .then(trendingData => {
            settrendingData(trendingData)
            console.log(trendingData)
    ...
    const getData = async(pageCount)=>{
        setTimeout(() => setLoading(true), 2000);
        const trendingDataResponse = await fetch(`${env.apiUrl}/trending/${mediaType}/${timeWindow}?api_key=${env.tmdbApiKey}&page=${pageCount}`)
    ...
    const changeTimeWindowType = (e)=>{  
        ...
        console.log(`${env.apiUrl}/trending/${mediaType}/${setTimeWindowType}?api_key=${env.tmdbApiKey}`)
        fetch(`${env.apiUrl}/trending/${mediaType}/${setTimeWindowType}?api_key=${env.tmdbApiKey}`)
        .then(response => response.json())
        .then(trendingData => {
            settrendingData(trendingData)
            console.log(trendingData)
    ...
    return(
        <>
            <Head>
                <title>Trending Tv Shows::Media</title>
                    ...
                    <MainContainer>
                        <div className="row_movie_wrapper"> 
                            <div className="main_grid_wrapper">
                                    ...
                                    <button onClick={changeMediaType} value="tv">tvs</button>
...
//     const tvtrendingData = await responsstvPopular.json();
//     return{
//       props:{
//         tvtrendingData: tvtrendingData,
//       }
//     }
//   }