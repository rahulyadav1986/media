...
const TopRated = ()=>{
    ...
    const url = `${env.apiUrl}/movie/top_rated?api_key=${env.tmdbApiKey}`;
    const NextPage = ()=>{
    ...
    useEffect(()=>{
        setTimeout(() => setLoading(true), 2000);
        fetch(`${env.apiUrl}/movie/top_rated?api_key=${env.tmdbApiKey}&page=1`)
        .then(response => response.json())
        .then(topRatedData => {
            settopRatedData(topRatedData)
            console.log(topRatedData)
    ...
    const getData = async(pageCount)=>{
        setTimeout(() => setLoading(true), 2000);
        const topRatedDataResponse = await fetch(`${env.apiUrl}/movie/top_rated?api_key=${env.tmdbApiKey}&page=${pageCount}`)
