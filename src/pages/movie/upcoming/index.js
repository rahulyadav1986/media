...
const Upcoming = ()=>{
    ...
    const url = `${env.apiUrl}/movie/upcoming?api_key=${env.tmdbApiKey}`;
    const NextPage = ()=>{
    ...
    useEffect(()=>{
        setTimeout(() => setLoading(true), 2000);
        fetch(`${env.apiUrl}/movie/upcoming?api_key=${env.tmdbApiKey}&page=1`)
        .then(response => response.json())
        .then(upcoimgData => {
            setupcoimgData(upcoimgData)
            console.log(upcoimgData)
    ...
    const getData = async(pageCount)=>{
        setTimeout(() => setLoading(true), 2000);
        const upcoimgDataResponse = await fetch(`${env.apiUrl}/movie/upcoming?api_key=${env.tmdbApiKey}&page=${pageCount}`)
    ...
    return(
        <>
            <Head>
                <title>Upcoming Movies::Media</title>
