                    ...
                    HeroData?.results.map((item,i)=>{
                        return(
                            ...
                            <div key={i} className={`${styles.main_hero_item} ${tab === i ? `${styles.active}`: ``}`}>
                                <Image alt={item.title} className={styles.avator} src={`${enviourment.image_base_url}/original${item.backdrop_path}`}  fill={true} />
                                    ...
                                    <MainContainer>
                                        ...
                                        <ul className={`${styles.widget_area} d-flex align-items-center`}>
                                            ...
                                            <li className="d-flex align-items-center">
                                                <Image alt="icon" className={styles.icon} src="/images/like.png" fill={true} /> {item.vote_count}
                                            </li>
                                            <li className="d-flex align-items-center">
                                                <Image alt="icon" className={styles.icon} src="/images/date.png" fill={true} />
                                    ...
                                    HeroData.results.slice(0,4).map((item,i)=>{
                                        return(
                                            <li key={i} onClick={() => setTab(i)} className={tab === i ? `${styles.active}`: ""}>
                                                <Image alt={item.title} src={`${enviourment.image_base_url}/w300${item.poster_path}`} fill={true} />
                                            </li>
                                        )
                                    })
                                }
