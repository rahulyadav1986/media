...
const Casting = ({tvDetailsCredits}) =>{
    const breakPoints = [
    ...
    return(
        <>
        {
            tvDetailsCredits.cast.length > 0 ?
            <div className={styles.casting_wrap}>                
                <h3>Casting</h3>
                <div className={styles.details}>
                    {
                        tvDetailsCredits.cast.length < 7 ?
                        <div className="cast_grid">
                            {
                                tvDetailsCredits.cast.map((item,i)=>{
                                    return(
                                        <>
                                            <Link href={`/tv/person/${item.id}`}key={i} className={styles.card_details}>
